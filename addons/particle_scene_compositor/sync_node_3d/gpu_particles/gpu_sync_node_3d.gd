...
## If true, all child [GPUParticles3D] nodes will only emit once. Otherise, they will loop
## and restart once all [GPUParticles3D] have finished or [member time_to_finish] is reached.
...
## to this value or higher. Otherwise, it will finish once all child [GPUParticles3D]
## emit their [signal GPUParticles3D.finished] signals. Finishing will stop any ongoing child [GPUParticles3D]
## emissions.
## [br][br]
## Useful when subemitters are used, since their [signal GPUParticles3D.finished] signal is
...
signal finished
## Emitted after all child [GPUParticles3D] nodes emit their [signal GPUParticles3D.finished] signal
...
## Restarts all child [GPUParticles3D], interrupting any in-progress emissions.
## Any newly-added [GPUParticles3D] nodes will be automatically included.
## [br][br]
## Optional [param preprocess] time can be passed to advance all effects from the start,
## such as when loading a saved game state
...
## Stops all [GPUParticles3D] emissions, interrupting any in-progress emissions.
...
static func _activate(node: GPUParticles3D) -> void:
	node.one_shot = true
	node.emitting = true
	node.restart()
