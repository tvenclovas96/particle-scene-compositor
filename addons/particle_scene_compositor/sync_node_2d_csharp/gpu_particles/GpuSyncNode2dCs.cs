...
/// <summary>
/// Synchronization hub node that starts and tracks all child <see cref="GpuParticles2D"/>.
/// It will detect any <see cref="GpuParticles2D"/> if it is their ancestor in the tree; they do not need
...
/// </summary>
[GlobalClass, Icon("res://addons/particle_scene_compositor/sync_node_2d_csharp/gpu_particles/GpuSyncNode2dCs.cs")]
public partial class GpuSyncNode2dCs : Node2D
{
    /// <summary>
    /// If <see langword="true"/>, the node will automatically start all child <see cref="GpuParticles2D"/> on ready
    /// </summary>
    [Export] public bool Autostart { get; set; } = true;

    /// <summary>
    /// If <see langword="true"/>, all child <see cref="GpuParticles2D"/> will emit only once. Otherwise,
    /// they will loop and restart once all <see cref="GpuParticles2D"/> have finished or <see cref="TimeToFinish"/>
    /// is reached.
    /// </summary>
    ...
    /// to this value or higher. Otherwise, it will finish once all child <see cref="GpuParticles2D"/>
    /// emit their <see cref="GpuParticles2D.Finished"/> signals. Finishing will stop any ongoing child
    /// <see cref="GpuParticles2D"/> emissions.
    /// 
    /// <para> Useful when subemitters are used, since their <see cref="GpuParticles2D.Finished"/>
    ...
    /// <summary>
    /// is <see langword="true"/> if any child <see cref="GpuParticles2D"/> is currently emitting
    /// </summary>
    ...
    /// while no child <see cref="GpuParticles2D"/> are currently emitting.</para>
    /// </summary>
    ...
    /// <summary>
    /// Emitted if <see cref="OneShot"/> is <see langword="true"/> and all child <see cref="GpuParticles2D"/>
    ...
    /// <summary>
    /// Emitted after all child <see cref="GpuParticles2D"/> emit their <see cref="GpuParticles2D.Finished"/> signal
    ...
    /// <summary>
    /// Starts all child <see cref="GpuParticles2D"/>. Automatically called on <see cref="_Ready"/> if autostart is enabled.
    ...
    /// <summary>
    /// Restarts all child <see cref="GpuParticles2D"/>, interrupting any in-progress emissions.
    /// Any newly added child <see cref="GpuParticles2D"/> nodes will be automatically included.
    /// <para> Optional <see cref="preprocess"/> parameter can be passed to advance all effects from emission start,
    /// such as when loading a saved game state </para>
    /// </summary>
    /// <param name="preprocess"></param>
    ...
    /// <summary>
    /// Stops all child <see cref="GpuParticles2D"/> emissions, interrupting any in-progress emissions.
    ...
    private static void Activate(GpuParticles2D particles)
    {
    ...
    {
        if (node is GpuParticles2D particles) particles.Emitting = false;
