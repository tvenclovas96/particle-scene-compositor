...
/// </summary>
[GlobalClass, Icon("res://addons/particle_scene_compositor/sync_node_3d_csharp/gpu_particles/Gpu3d.svg")]
public partial class GpuSyncNode3dCs : Node3D
{
    /// <summary>
    /// If <see langword="true"/>, the node will automatically start all child <see cref="GpuParticles3D"/> on ready
    /// </summary>
    [Export] public bool Autostart { get; set; } = true;

    /// <summary>
    /// If <see langword="true"/>, all child <see cref="GpuParticles3D"/> will emit only once. Otherwise,
    /// they will loop and restart once all <see cref="GpuParticles3D"/> have finished or <see cref="TimeToFinish"/>
    /// is reached.
    /// </summary>
    ...
    /// to this value or higher. Otherwise, it will finish once all child <see cref="GpuParticles3D"/>
    /// emit their <see cref="GpuParticles3D.Finished"/> signals. Finishing will stop any ongoing child
    /// <see cref="GpuParticles3D"/> emissions.
    /// 
    /// <para> Useful when subemitters are used, since their <see cref="GpuParticles3D.Finished"/>
    ...
    /// <summary>
    /// is <see langword="true"/> if any child <see cref="GpuParticles3D"/> is currently emitting
    /// </summary>
    ...
    /// <summary>
    /// Emitted if <see cref="OneShot"/> is <see langword="true"/> and all child <see cref="GpuParticles3D"/>
    ...
    /// <summary>
    /// Emitted after all child <see cref="GpuParticles3D"/> emit their <see cref="GpuParticles3D.Finished"/> signal
    ...
    /// <summary>
    /// Starts all child <see cref="GpuParticles3D"/>. Automatically called on <see cref="_Ready"/> if autostart is enabled.
    ...
    /// <summary>
    /// Restarts all child <see cref="GpuParticles3D"/>, interrupting any in-progress emissions.
    /// Any newly added child <see cref="GpuParticles3D"/> nodes will be automatically included.
    /// <para> Optional <see cref="preprocess"/> parameter can be passed to advance all effects from emission start,
    /// such as when loading a saved game state </para>
    /// </summary>
    /// <param name="preprocess"></param>
    ...
    /// <summary>
    /// Stops all child <see cref="GpuParticles3D"/> emissions, interrupting any in-progress emissions.
    ...
    private static void Activate(GpuParticles3D particles)
    {
    ...
    private void RecursiveRestart(Node node, float preprocess)
    {
        if (node is GpuParticles3D particles)
        {
            particles.Preprocess = preprocess;
            Activate(particles);
            if (!IsConnected(GpuParticles3D.SignalName.Finished, Callable.From(Decrement)))
                particles.Finished += Decrement;
        }
        {
            foreach (Node child in node.GetChildren())
